...
pub fn main() {
	...
	'main_loop: loop {
        ...
        if input_manager.get_key_down(Keycode::Up) {canvas.set_draw_color(Color::RGB(0, 255, 0))}
        else if input_manager.get_key_down(Keycode::Down) {canvas.set_draw_color(Color::RGB(0, 0, 255))}
        else {canvas.set_draw_color(Color::RGB(0, 0, 0))}
